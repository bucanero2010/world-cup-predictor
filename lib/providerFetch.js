...
// Last observed usage from the provider, per key index (0 = primary, 1 = backup).
// Captured from the x-requests-remaining response header on each successful call.
let lastUsage = null; // { keyIndex, remaining }

/** Most recent provider usage observation, or null if no call has been made yet. */
...
async function getJson(url) {
  ...
  const remaining = remHeader == null ? null : Number(remHeader);
...
export async function fetchWithFailover(buildUrlForKey) {
  ...
  for (let i = 0; i < keys.length; i++) {
    try {
      const { json, remaining } = await getJson(buildUrlForKey(keys[i]));
      if (Number.isFinite(remaining)) lastUsage = { keyIndex: i, remaining };
      return json;
    } catch (err) {
      lastErr = err;
