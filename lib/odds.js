...
/** P(total goals > line) for a grid, for an arbitrary line. */
function probOver(grid, line) {
  let pOver = 0;
  for (const [key, p] of grid) {
    const [h, a] = key.split("-").map(Number);
    if (h + a > line) pOver += p;
 ...
 * @param {Array<{line:number, overProb:number}>} [input.totalsLines] when provided,
 *        the solver also fits these de-vigged over-probabilities (the alt-totals ladder),
 *        giving a much tighter goal-distribution constraint than a single line.
 * @returns {{lambdaHome:number, lambdaAway:number, target:object, method:string, cost:number}}
 */
export function fromOdds({ oneXtwo, totalLine = 2.5, overUnder, rho = -0.05, devig: devigFn, totalsLines }) {
  ...
  const hasLadder = Array.isArray(totalsLines) && totalsLines.length > 0;
  ...
  const search = (loH, hiH, loA, hiA, step) => {
    for (let lh = loH; lh <= hiH; lh += step) {
      for (let la = loA; la <= hiA; la += step) {
        ...
        const s = summarize(grid, totalLine);
        let cost =
          (s.pHome - pH) ** 2 + (s.pDraw - pD) ** 2 + (s.pAway - pA) ** 2;
        if (hasLadder) {
          // Fit the whole alt-totals ladder instead of a single line.
          for (const { line, overProb } of totalsLines) {
            cost += (probOver(grid, line) - overProb) ** 2;
          }
        } else if (pOverTarget !== null) {
          cost += (s.pOver - pOverTarget) ** 2;
        }
  ...
  // Market consistency check (per review): recompute the implied 1X2 + over from the
  // solved grid and compare to the de-vigged market targets. A large residual means the
  // 1X2 and totals markets point in inconsistent directions and no single λ fits both —
  // surface it instead of silently returning a poor fit.
  const finalGrid = scoreGrid(best.lambdaHome, best.lambdaAway, rho);
  const s = summarize(finalGrid, totalLine);
  const resid = {
    home: s.pHome - pH,
    draw: s.pDraw - pD,
    away: s.pAway - pA,
    over: pOverTarget === null ? null : probOver(finalGrid, totalLine) - pOverTarget,
  };
  const outcomeResiduals = [resid.home, resid.draw, resid.away].map(Math.abs);
  const maxOutcomeResidual = Math.max(...outcomeResiduals); // L∞ over home/draw/away
  // "consistent" when every outcome is within 3pp of the market (review's threshold).
  const consistent = maxOutcomeResidual <= 0.03;
  ...
  return {
    ...
    cost: best.cost,
    fit: {
      implied: { home: s.pHome, draw: s.pDraw, away: s.pAway },
      residuals: resid,
      maxOutcomeResidual,
      consistent,
    },
